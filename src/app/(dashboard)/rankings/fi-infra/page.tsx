...
function sortRows(rows: any[], key: string, dir: string) {
  return [...rows].sort((a, b) => {
    const av = a[key] ?? null; const bv = b[key] ?? null
...
async function FiInfraTable({ sortKey, dir }: { sortKey: string; dir: string }) {
  ...
  const rawRows = (assets ?? []).map(a => {
    ...
    return {
      ...
      avg_volume: m.avg_volume ?? null, score: m.score ?? 0,
    ...
    <Card className="overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.length === 0 && (
              <tr><td colSpan={9} className="px-4 py-8 text-center text-muted-foreground">Nenhum dado disponível.</td></tr>
            )}
            {rows.map((row, idx) => (
              <tr key={row.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                <td className="px-4 py-3 text-muted-foreground font-medium">{idx + 1}</td>
                <td className="px-4 py-3"><p className="font-bold">{row.ticker}</p><p className="text-xs text-muted-foreground">{row.name}</p></td>
                <td className="px-4 py-3"><Badge variant="secondary" className="text-xs">{row.segment ?? 'Infraestrutura'}</Badge></td>
...
export default async function FiInfraRankingPage({ searchParams }: { searchParams: Promise<{ sort?: string; dir?: string }> }) {
  const { sort: sortKey = 'score', dir = 'desc' } = await searchParams
  return (
    <div className="space-y-5 max-w-full">
      <div>
        <h1 className="text-2xl font-bold flex items-center gap-2"><Zap className="h-6 w-6 text-primary" />Ranking de FI-Infra</h1>
        <p className="text-muted-foreground text-sm mt-1">Clique em qualquer coluna para ordenar. Fundos isentos de IR para PF, lastreados em debêntures incentivadas.</p>
      </div>
      <Suspense fallback={<div className="h-64 flex items-center justify-center text-muted-foreground">Carregando…</div>}>
        <FiInfraTable sortKey={sortKey} dir={dir} />
      </Suspense>
